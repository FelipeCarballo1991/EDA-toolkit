...
class PipeReader(DelimitedTextReader):
    """
    Pipe-Separated Values file reader.
    
    Extends DelimitedTextReader with pipe (|) as the primary delimiter.
    ...
    >>> df = reader.read("data.txt", normalize=True, normalize_columns=True)
    """
    ...
    def __init__(
        ...
        Pipe reader prioritizes pipe delimiter but can fallback to others.
        ...
        """
        # Default delimiters: pipe first, then fallback options
